// File: src/app/quantitative/page.tsx
'use client';
...
const QuantitativePage = () => {
  ...
  const fetchTopics = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/quantitative?userId=${user.id}`);
  ...
  return (
    <div className="p-6 bg-white max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Quantitative Practice</h1>
      
      {/* Practice button 
      <div className="mb-6">
        <Link href="/quantitative/practice">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {topics.map((topic) => (
          <Link 
            href={`/quantitative/topics/${topic.id}`} 
            key={topic.id}
          >
