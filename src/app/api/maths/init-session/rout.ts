...
// This is similar to the quantitative version but uses math-specific tables
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newTestAttempt = await db.insert(mathTestAttempts).values({
      userId,
      subtopicId,
      startTime: new Date(),
      status: 'in_progress',
    ...
    return NextResponse.json({ 
      success: true,
      message: "Math test session initialized successfully",
      testAttemptId
    });
    ...
    return NextResponse.json(
      { error: "Failed to initialize math test session", details: errorMessage },
      { status: 500 }
    );
  }
}