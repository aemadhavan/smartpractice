...
export type Option = {
  ...
  export type Question = {
    id: number;
    question: string;
    options: any; // Could be string, array, or object depending on source
  ...
  export function parseQuestionOptions(question: Question): ProcessedQuestion {
    ...
    try {
      ...
      // Handle object format (could be {0: {}, 1: {}} or {A: {}, B: {}})
        ...
        parsedOptions = Object.values(rawOptions).map(normalizeOption);
      }
   ...
   */
  function normalizeOption(opt: any): Option {
    ...
    return {
      id: opt.id || opt.value || opt.key || String(Math.random()),
